...
void FitDomain::setParameterFixed(std::string const &parameter,
                                  bool fix) const {
  if (!hasParameter(parameter))
...
bool FitDomain::isParameterFixed(std::string const &parameter) const {
  if (hasParameter(parameter)) {
    return m_function->getParameterStatus(m_function->parameterIndex(
               parameter)) == IFunction::ParameterStatus::Fixed;
  }
  throw std::runtime_error("The parameter does not exist in this domain.");
}
...
std::string
FitDomain::getParameterConstraint(std::string const &parameter) const {
  if (hasParameter(parameter)) {
    auto const parameterIndex = m_function->parameterIndex(parameter);
