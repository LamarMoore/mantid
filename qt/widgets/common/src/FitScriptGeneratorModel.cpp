...
std::string getParameterName(std::string const &constraint) {
  return splitParameterName(splitConstraintString(constraint).first)
      .second.toStdString();
}
...
std::string getAdjustedConstraint(std::string const &constraint) {
  ...
  return limits.first.toStdString() + "<" + parameterName + "<" +
...
std::string
FitScriptGeneratorModel::getFullParameter(FitDomainIndex domainIndex,
                                          std::string const &parameter) const {
  if (m_fittingMode == FittingMode::SEQUENTIAL)
    return parameter;
...
std::string FitScriptGeneratorModel::getFullTie(FitDomainIndex domainIndex,
...
void FitScriptGeneratorModel::updateParameterConstraint(
  ...
  m_fitDomains[domainIndex.value]->updateParameterConstraint(
      getAdjustedFunctionIndex(functionIndex), getParameterName(constraint),
      constraint);
}
...
std::string
FitScriptGeneratorModel::getDomainName(FitDomainIndex domainIndex) const {
  if (domainIndex.value < numberOfDomains())
...
bool FitScriptGeneratorModel::hasParameter(
    FitDomainIndex domainIndex, std::string const &fullParameter) const {
  return getParameterProperty(&FitDomain::hasParameter, domainIndex,
                              fullParameter);
}

void FitScriptGeneratorModel::setParameterValue(
    FitDomainIndex domainIndex, std::string const &fullParameter,
    double value) {
  auto const parameter = getAdjustedFunctionIndex(fullParameter);
  if (domainIndex.value >= numberOfDomains())
...
void FitScriptGeneratorModel::setParameterFixed(
    FitDomainIndex domainIndex, std::string const &fullParameter, bool fix) {
  auto const parameter = getAdjustedFunctionIndex(fullParameter);
  if (domainIndex.value >= numberOfDomains())
    throw std::runtime_error("The domain index provided does not exist.");
...
void FitScriptGeneratorModel::setParameterTie(FitDomainIndex domainIndex,
                                              std::string const &fullParameter,
...
void FitScriptGeneratorModel::setParameterConstraint(
    FitDomainIndex domainIndex, std::string const &fullParameter,
  ...
  if (!constraint.empty()) {
    m_fitDomains[domainIndex.value]->updateParameterConstraint(
        getFunctionIndex(parameter), getParameterName(constraint),
        getAdjustedConstraint(constraint));
  } else {
...
double FitScriptGeneratorModel::getParameterValue(
    FitDomainIndex domainIndex, std::string const &fullParameter) const {
  return getParameterProperty(&FitDomain::getParameterValue, domainIndex,
                              fullParameter);
}

bool FitScriptGeneratorModel::isParameterFixed(
    FitDomainIndex domainIndex, std::string const &fullParameter) const {
  return getParameterProperty(&FitDomain::isParameterFixed, domainIndex,
                              fullParameter);
}

std::string FitScriptGeneratorModel::getParameterTie(
    FitDomainIndex domainIndex, std::string const &fullParameter) const {
  return getParameterProperty(&FitDomain::getParameterTie, domainIndex,
                              fullParameter);
}

std::string FitScriptGeneratorModel::getParameterConstraint(
    FitDomainIndex domainIndex, std::string const &fullParameter) const {
  return getParameterProperty(&FitDomain::getParameterConstraint, domainIndex,
                              fullParameter);
}
