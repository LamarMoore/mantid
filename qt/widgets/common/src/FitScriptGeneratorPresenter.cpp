...
void FitScriptGeneratorPresenter::handleEditLocalParameterClicked(
    std::string const &parameter) {
  std::vector<std::string> workspaceNames, domainNames, ties, constraints;
  std::vector<double> values;
  std::vector<bool> fixes;
  insertLocalParameterData(parameter, workspaceNames, domainNames, values,
                           fixes, ties, constraints);

  m_view->openEditLocalParameterDialog(
      m_model->getAdjustedFunctionIndex(parameter), workspaceNames, domainNames,
      values, fixes, ties, constraints);
}

void FitScriptGeneratorPresenter::handleEditLocalParameterFinished() {
  auto const [parameter, values, fixes, ties, constraints] =
      m_view->getEditLocalParameterResults();

  auto const domainIndices = getDomainsWithLocalParameter(parameter);
  for (auto i = 0u; i < values.size(); ++i) {
    setLocalParameterDataForDomain(domainIndices[i], parameter, values[i],
                                   fixes[i], ties[i], constraints[i]);
  }
...
void FitScriptGeneratorPresenter::updateParameterValue(
...
void FitScriptGeneratorPresenter::updateAttributeValue(
    std::string const &workspaceName, WorkspaceIndex workspaceIndex,
    std::string const &fullAttribute, IFunction::Attribute const &newValue) {
  m_model->updateAttributeValue(workspaceName, workspaceIndex, fullAttribute,
                                newValue);
}
...
void FitScriptGeneratorPresenter::insertLocalParameterData(
    std::string const &parameter, std::vector<std::string> &workspaceNames,
  ...
  for (auto const domainIndex : domainIndices) {
    insertLocalParameterDataForDomain(domainIndex, parameter, workspaceNames,
                                      domainNames, values, fixes, ties,
                                      constraints);
  }
}
...
void FitScriptGeneratorPresenter::insertLocalParameterDataForDomain(
  ...
  if (m_model->hasParameter(domainIndex, parameter)) {
    ...
    constraints.emplace_back(
        m_model->getParameterConstraint(domainIndex, parameter));
  }
}

void FitScriptGeneratorPresenter::setLocalParameterDataForDomain(
    FitDomainIndex domainIndex, std::string const &parameter, double value,
    bool fix, std::string const &tie, std::string const &constraint) {
...
FitScriptGeneratorPresenter::getDomainsWithLocalParameter(
    std::string const &parameter) const {
  auto const doesNotHaveParameter = [&](FitDomainIndex domainIndex) {
    return !m_model->hasParameter(
        domainIndex, m_model->getFullParameter(domainIndex, parameter));
  };

  auto domainIndices = m_view->allRows();
  domainIndices.erase(std::remove_if(domainIndices.begin(), domainIndices.end(),
                                     doesNotHaveParameter),
                      domainIndices.end());
