...
public:
  ...
  void test_that_the_FitDomain_has_been_instantiated_with_the_correct_data() {
    TS_ASSERT_EQUALS(m_fitDomain->domainName(),
                     m_wsName + " (" + std::to_string(m_wsIndex.value) + ")");
  ...
  test_that_setParameterFixed_will_throw_if_the_parameter_if_the_parameter_does_not_exist_when_in_sequential_mode() {
    ...
    TS_ASSERT_THROWS(m_fitDomain->setParameterFixed("f0.BadParam", true),
                     std::runtime_error const &);
  }
  ...
  test_that_updateParameterConstraint_will_add_a_constraint_as_expected_to_a_composite_function() {
    ...
    TS_ASSERT_EQUALS(m_fitDomain->getParameterConstraint("f1.Height"),
                     constraint);
  }
